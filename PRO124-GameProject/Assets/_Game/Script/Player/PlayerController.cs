...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Special Ability 1 - Projectile")]
    ...
    [Header("Special Ability 2 - Projectile")]
    public GameObject invincibleEffectPrefab; // Kéo vào từ Inspector
    public Transform effectSpawnPoint; // Nơi xuất hiện hiệu ứng
    public float ability2ManaCost = 40f;
    public float ability2Cooldown = 20f;
    ...
    void HandleGroundCollision()
    {
        ...
        if (isAirSlamming && isGrounded)
        {
            ...
            isAirSlamming = false; GameObject aoEInstance = Instantiate(aoEPrefab, spawnPos, Quaternion.identity);
        ...
        {
            GameObject aoEInstance = Instantiate(dustPrefab, spawnPos, Quaternion.identity);
            StartCoroutine(DestroyAfterDelay(aoEInstance, 0.2f));
    ...
    {
        // Kiểm tra cooldown, grounded và mana
        if (Input.GetKeyDown(KeyCode.Alpha1) && isGrounded && ability1Timer <= 0f)
        {
            PlayerStats stats = GetComponent<PlayerStats>();
            if (stats != null && stats.UseMana(ability1ManaCost))
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
                animator.SetTrigger("isUsingSpecialAbility1");
                StartCoroutine(DelayedShoot(0.4f));
                ability1Timer = ability1Cooldown; // Đặt lại thời gian hồi chiêu
            }
            else
            {
                Debug.Log("Không đủ mana để dùng kỹ năng 1");
            }
        }
    }
    ...
    { // Wait for the animation to play before firing
        if (projectilePrefab == null || firePoint == null) return;

        GameObject proj = Instantiate(projectilePrefab, firePoint.position, Quaternion.identity);

        // Lấy hướng theo mặt nhân vật
    ...
    {
        if (Input.GetKeyDown(KeyCode.Alpha2) && isGrounded && ability2Timer <= 0f)
        {
            PlayerStats stats = GetComponent<PlayerStats>();
            if (stats != null && stats.UseMana(ability2ManaCost))
            {
                StartCoroutine(AttackMoveLock(attackMoveLockDuration));
                animator.SetTrigger("isUsingSpecialAbility2");

                // Tạo hiệu ứng bất tử
                GameObject effect = Instantiate(invincibleEffectPrefab, effectSpawnPoint.position, Quaternion.identity, transform);
                Destroy(effect, 5f);

                // Bật trạng thái bất tử
                stats.SetInvincibility(5f);

                ability2Timer = ability2Cooldown; // Đặt lại thời gian hồi chiêu
            }
            else
            {
                Debug.Log("Không đủ mana để dùng kỹ năng 2");
            }
        }
    }
